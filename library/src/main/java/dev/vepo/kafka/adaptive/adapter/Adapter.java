package dev.vepo.kafka.adaptive.adapter;

import static dev.vepo.kafka.adaptive.AdaptiveConfigs.ADAPTIVE_ADAPTER_FREQUENCY_MS_CONFIG;
import static dev.vepo.kafka.adaptive.AdaptiveConfigs.ADAPTIVE_ADAPTER_HISTORY_SIZE_MAX_CONFIG;
import static dev.vepo.kafka.adaptive.AdaptiveConfigs.ADAPTIVE_ADAPTER_HISTORY_SIZE_MIN_CONFIG;
import static dev.vepo.kafka.adaptive.AdaptiveConfigs.DEFAULT_ADAPTIVE_ADAPTER_HISTORY_SIZE_MAX;
import static dev.vepo.kafka.adaptive.AdaptiveConfigs.DEFAULT_ADAPTIVE_ADAPTER_HISTORY_SIZE_MIN;
import static dev.vepo.kafka.adaptive.adapter.stats.StatsKey.broker;
import static dev.vepo.kafka.adaptive.adapter.stats.StatsKey.client;
import static dev.vepo.kafka.adaptive.adapter.stats.StatsKey.jvm;
import static dev.vepo.kafka.adaptive.adapter.stats.StatsKey.partition;
import static dev.vepo.kafka.adaptive.adapter.stats.StatsKey.topic;
...
import dev.vepo.kafka.adaptive.Streams;
import dev.vepo.kafka.adaptive.adapter.context.ResourcesState;
import dev.vepo.kafka.adaptive.adapter.context.StreamsContext;
import dev.vepo.kafka.adaptive.adapter.context.ThroughputState;
import dev.vepo.kafka.adaptive.adapter.rules.AdapterRule;
import dev.vepo.kafka.adaptive.adapter.rules.AdapterRule.RequiredChanges;
import dev.vepo.kafka.adaptive.adapter.rules.AdjustConsumerFetchSizeRule;
import dev.vepo.kafka.adaptive.adapter.rules.BatchProducerRule;
import dev.vepo.kafka.adaptive.adapter.rules.ThreadAllocationRule;
import dev.vepo.kafka.adaptive.adapter.rules.ThroughputAnalyzerRule;
import dev.vepo.kafka.adaptive.adapter.rules.UseCompressionOnProducerRule;
import dev.vepo.kafka.adaptive.adapter.stats.StatsKey;
import dev.vepo.kafka.adaptive.adapter.stats.StatsValues;
import dev.vepo.kafka.adaptive.metrics.BrokerMetricsCollector;
import dev.vepo.kafka.adaptive.metrics.MetricListener;
import dev.vepo.kafka.adaptive.metrics.MetricsRecorder;
import dev.vepo.kafka.adaptive.metrics.MetricsRecorder.Event;
import dev.vepo.kafka.adaptive.metrics.PerformanceMetric;
...
public class Adapter implements MetricListener, Configurable, StateListener {
    ...
    public Adapter() {
        this.maxHistorySize = DEFAULT_ADAPTIVE_ADAPTER_HISTORY_SIZE_MAX;
        this.minHistorySize = DEFAULT_ADAPTIVE_ADAPTER_HISTORY_SIZE_MIN;
    ...
    public void configure(Map<String, ?> props) {
        ...
        var configuredRules = configs.getConfiguredInstances(AdaptiveConfigs.ADAPTER_RULE_CLASSES_CONFIG, AdapterRule.class);
