package dev.vepo.adaptive.experiment.stream;
...
import dev.vepo.adaptive.experiment.stream.CommandClient.Command;
import dev.vepo.adaptive.experiment.stream.model.FareStats;
import dev.vepo.adaptive.experiment.stream.model.PassengerStats;
import dev.vepo.adaptive.experiment.stream.model.TaxiTrip;
import dev.vepo.adaptive.experiment.stream.model.TipStats;
import dev.vepo.adaptive.experiment.stream.model.TripStats;
import dev.vepo.adaptive.experiment.stream.serdes.JsonSerde;
import dev.vepo.kafka.adaptive.AdaptiveConfigs;
import dev.vepo.kafka.adaptive.AdaptiveStreams;
import dev.vepo.kafka.adaptive.Streams;
import dev.vepo.kafka.adaptive.VanillaStreams;
...
import dev.vepo.kafka.adaptive.metrics.PerformanceMetricsCollector;
...
public class StreamPerformanceEvaluation implements Runnable {
    ...
    private static final int TEST_DURATION_IN_MINUTES = 12;
    ...
    public void run() {
        ...
        try (var adaptiveStream = create(this::buildTopology, props);
    ...
    private Streams create(Supplier<Topology> topologyProvider, Properties props) {
        return switch (this.type) {
            ...
            case ADAPTIVE -> new AdaptiveStreams(topologyProvider, props);
