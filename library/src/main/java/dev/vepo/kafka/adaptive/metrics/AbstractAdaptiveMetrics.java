...
import dev.vepo.kafka.adaptive.metrics.PerformanceMetric.Context;

public abstract class AbstractAdaptiveMetrics implements MetricsReporter {
    private static final Logger logger = LoggerFactory.getLogger(AbstractAdaptiveMetrics.class);
    ...
    protected AbstractAdaptiveMetrics() {
    ...
    public void configure(Map<String, ?> configs) {
        ...
        var mConfigs = new AdaptiveConfigs(configs);
        ...
        if (!clientConfig.endsWith("-admin")) {
            var frequencyInMs = mConfigs.getLong(AdaptiveConfigs.ADAPTIVE_METRICS_COLLECTOR_FREQUENCY_MS_CONFIG);
